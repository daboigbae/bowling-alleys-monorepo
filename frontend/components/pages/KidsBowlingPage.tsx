...
import {
  getKidFriendlyStates,
  getKidFriendlyVenuesByState,
  Venue,
} from "@/lib/firestore";
...
export default function KidsBowling({ state: propState, city: propCity }: KidsBowlingPageProps = {}) {
  ...
  useEffect(() => {
    ...
    if (displayCity && displayState) {
      title = `Kids Bowling in ${displayCity}, ${displayState} | Family-Friendly Bowling`;
      description = `Find the best kid-friendly bowling alleys in ${displayCity}. Family fun with bumpers, lightweight balls, and arcade games — check amenities, hours, and pricing near you.`;
    } else if (displayState) {
      title = `Kids Bowling in ${displayState} | Family-Friendly Bowling Alleys`;
      description = `Explore kid-friendly bowling alleys across ${displayState}. Find venues with bumper bowling, family packages, and entertainment for all ages.`;
    } else {
      title = "Kids Bowling Near You | Family-Friendly Bowling Alleys";
      description = "Find the best kid-friendly bowling alleys in the U.S. Discover venues with bumper lanes, lightweight balls, arcade games, and family packages.";
    }
    ...
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 2,
            name: "Kids Bowling",
            item: `${window.location.origin}/kids-bowling`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Kids Bowling`,
                    item: `${window.location.origin}/kids-bowling/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Kids Bowling`,
                    item: `${window.location.origin}/kids-bowling/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Kids Bowling in ${displayCity}, ${displayState}`
            : displayState
              ? `Kids Bowling in ${displayState}`
              : "Kids Bowling by State",
        description: description,
      },
    };

    const existingScript = document.querySelector(
      'script[type="application/ld+json"][data-kids-bowling]',
    );
    if (existingScript) {
    ...
    script.setAttribute("data-kids-bowling", "true");
    script.textContent = JSON.stringify(structuredData);
    document.head.appendChild(script);
  }, [displayState, displayCity, selectedState, selectedCity]);

  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/kids-bowling-states"],
    queryFn: getKidFriendlyStates,
  });

  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/kids-bowling-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getKidFriendlyVenuesByState(displayState!)
        : Promise.resolve([]),
  ...
  const handleVenueClick = (venueId: string) => {
    ...
    trackEvent("kids_bowling_venue_click", "navigation", venue?.name || venueId);
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-kids-bowling-title"
            >
              Browse Kids Bowling by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find kid-friendly bowling alleys with bumper lanes, lightweight balls,
              arcade games, and family packages. Select a state to explore family-friendly
              venues organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            <nav className="flex items-center gap-2 mb-4 text-sm">
              ...
              <Link
                href={`/kids-bowling/${selectedState}`}
                className="text-primary hover:text-primary/80 underline"
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/kids-bowling/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-kids-bowling-title"
            >
              Kids Bowling in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby offering kid-friendly bowling`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} offering kid-friendly bowling`;
              })()}
            </p>
          </>
        ) : (
          <>
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/kids-bowling"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-kids-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/kids-bowling")}
                className="text-primary hover:text-primary/80"
                data-testid="button-back-to-kids-bowling"
              >
                <ArrowLeft className="w-4 h-4 mr-2" />
                All States
              </Button>
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-kids-bowling-title"
            >
              Kids Bowling in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see kid-friendly bowling venues. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search kid-friendly bowling venues in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-kids-bowling-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} kid-friendly bowling venues matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("kids_bowling_state_click", "navigation", state);
            setLocation(`/kids-bowling/${state.replace(/\s+/g, "-")}`);
          }}
          icon="👶"
          testIdPrefix="kids-bowling-state"
          description="Click to browse kid-friendly bowling"
          emptyStateTitle="No kid-friendly bowling locations found"
          emptyStateMessage="We're still building our database of kid-friendly bowling venues."
          emptyStateIcon="👶"
        />
      ) : !selectedCity ? (
        venuesLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  trackEvent(
                    "kids_bowling_city_click",
                    "navigation",
                    `${city.name}, ${displayState}`,
                  );
                  setLocation(`/kids-bowling/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-kids-bowling-city-${city.slug}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse kid-friendly bowling
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with kid-friendly bowling found
            </h3>
            <p className="text-muted-foreground">
              No kid-friendly bowling found in {displayState}.
            </p>
          </div>
        )
      ) : venuesLoading ? (
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">👶</div>
          <h3 className="text-lg font-semibold mb-2">
            No kid-friendly bowling venues found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No kid-friendly bowling found matching "${searchTerm}" in ${displayCity}, ${displayState}.`
              : `No kid-friendly bowling found in ${displayCity}, ${displayState}.`}
          </p>
        </div>
      )}
