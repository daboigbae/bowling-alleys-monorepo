...
interface BowlingPingPongPageProps { state?: string; city?: string; }
export default function BowlingPingPong({ state: propState, city: propCity }: BowlingPingPongPageProps = {}) {
  const [cityMatch, cityParams] = useRoute("/ping-pong/:state/:city");
  const [stateMatch, stateParams] = useRoute("/ping-pong/:state");
  const [baseMatch] = useRoute("/ping-pong");
  ...
  // Set page title and meta description with ping pong focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Bowling Alleys with Ping Pong Tables in ${displayCity}, ${displayState} | Ping Pong & Bowling`;
      description = `Discover bowling alleys with ping pong tables in ${displayCity}, ${displayState}. Find venues with both bowling lanes and ping pong tables for a complete entertainment experience.`;
    } else if (displayState) {
      title = `Bowling & Ping Pong in ${displayState} | Bowling Alleys with Ping Pong Tables by City`;
      description = `Browse bowling alleys with ping pong across ${displayState}. Find venues with ping pong tables and bowling lanes in cities throughout the state.`;
    } else {
      title = "Bowling & Ping Pong Near Me | Find Bowling Alleys with Ping Pong Tables by State";
      description =
        "Discover bowling alleys with ping pong tables across the United States. Find venues offering both bowling and ping pong in your state and city.";
    }
    ...
    // Add structured data for ping pong bowling organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            "@type": "ListItem",
            position: 2,
            name: "Bowling & Ping Pong",
            item: `${window.location.origin}/ping-pong`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Bowling & Ping Pong`,
                    item: `${window.location.origin}/ping-pong/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Bowling & Ping Pong`,
                    item: `${window.location.origin}/ping-pong/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Bowling & Ping Pong in ${displayCity}, ${displayState}`
            : displayState
              ? `Bowling & Ping Pong in ${displayState}`
              : "Bowling & Ping Pong by State",
        description: description,
      },
    };

    // Remove existing structured data for ping pong bowling
    const existingScript = document.querySelector(
      'script[type="application/ld+json"][data-arcade]',
    );
    if (existingScript) {
    ...
    script.setAttribute("data-arcade", "true");
    script.textContent = JSON.stringify(structuredData);
    document.head.appendChild(script);
  }, [displayState, displayCity, selectedState, selectedCity]);

  // Query for states list (only states with bowling & ping pong)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/ping pong-states"],
    queryFn: getPingPongStates,
  });

  // Query for venues in selected state (only venues with ping pong/pool)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/ping pong-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getPingPongVenuesByState(displayState!)
        : Promise.resolve([]),
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-ping pong-title"
            >
              Browse Bowling & Ping Pong by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find bowling alleys with ping pong tables in your area. 
              Select a state to see bowling and ping pong venues organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/ping-pong/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-ping pong-title"
            >
              Bowling & Ping Pong in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby with ping pong tables`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} with ping pong/ping pong tables`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/ping-pong"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-ping pong-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/ping-pong")}
                className="text-primary hover:text-primary/80"
                data-testid="button-back-to-ping pong"
              >
                <ArrowLeft className="w-4 h-4 mr-2" />
                All States
              </Button>
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-ping pong-title"
            >
              Bowling & Ping Pong in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see bowling alleys with ping pong tables. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search bowling & ping pong venues in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-ping pong-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} bowling & ping pong venues matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("ping pong_state_click", "navigation", state);
            setLocation(`/ping-pong/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🏓"
          testIdPrefix="ping pong-state"
          description="Click to browse bowling & ping pong"
          emptyStateTitle="No bowling & ping pong locations found"
          emptyStateMessage="We're still building our database of bowling alleys with ping pong tables."
          emptyStateIcon="🏓"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  trackEvent(
                    "ping pong_city_click",
                    "navigation",
                    `${city.name}, ${displayState}`,
                  );
                  setLocation(`/ping-pong/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-ping pong-city-${city.slug}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse bowling & ping pong
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with bowling & ping pong found
            </h3>
            <p className="text-muted-foreground">
              No bowling alleys with ping pong tables found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🏓</div>
          <h3 className="text-lg font-semibold mb-2">
            No ping pong bowling venues found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No ping pong bowling found matching "${searchTerm}" in ${displayCity}, ${displayState}.`
              : `No ping pong bowling found in ${displayCity}, ${displayState}.`}
          </p>
        </div>
      )}
