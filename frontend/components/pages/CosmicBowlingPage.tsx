...
export default function CosmicBowling({ state: propState, city: propCity }: CosmicBowlingPageProps = {}) {
  ...
  // Set page title and meta description with cosmic bowling focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Cosmic Bowling in ${displayCity}, ${displayState} | Lights, Music & Fun`;
      description = `Find the best cosmic bowling spots in ${displayCity}. Enjoy music, neon lights, and glowing pins — check amenities, hours, and pricing near you.`;
    } else if (displayState) {
      title = `Cosmic Bowling in ${displayState} | Late Night Bowling & Glow Lanes`;
      description = `Explore late-night and glow bowling alleys across ${displayState}. See which venues offer cosmic bowling, music nights, and all-ages fun.`;
    } else {
      title = "Cosmic Bowling Near You | Late Night & Glow Bowl Fun";
      description = "Find the best cosmic bowling alleys in the U.S. Experience neon lights, music, and glow-in-the-dark lanes. Discover where to bowl after dark.";
    }
    ...
    // Add structured data for cosmic bowling organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 2,
            name: "Cosmic Bowling",
            item: `${window.location.origin}/cosmic-bowling`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Cosmic Bowling`,
                    item: `${window.location.origin}/cosmic-bowling/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Cosmic Bowling`,
                    item: `${window.location.origin}/cosmic-bowling/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Cosmic Bowling in ${displayCity}, ${displayState}`
            : displayState
              ? `Cosmic Bowling in ${displayState}`
              : "Cosmic Bowling by State",
        description: description,
      },
    };

    // Remove existing structured data for cosmic bowling
    const existingScript = document.querySelector(
      'script[type="application/ld+json"][data-cosmic]',
    );
    if (existingScript) {
    ...
    script.setAttribute("data-cosmic", "true");
    script.textContent = JSON.stringify(structuredData);
    document.head.appendChild(script);
  }, [displayState, displayCity, selectedState, selectedCity]);

  // Query for states list (only states with cosmic bowling)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/cosmic-states"],
    queryFn: getCosmicStates,
  });

  // Query for venues in selected state (only venues with cosmic bowling)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/cosmic-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getCosmicVenuesByState(displayState!)
        : Promise.resolve([]),
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-cosmic-title"
            >
              Browse Cosmic Bowling by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find bowling alleys offering cosmic and glow bowling experiences
              in your area. Select a state to see cosmic bowling opportunities
              organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              ...
              <Link
                href={`/cosmic-bowling/${selectedState}`}
                className="text-primary hover:text-primary/80 underline"
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/cosmic-bowling/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-cosmic-title"
            >
              Cosmic Bowling in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby offering cosmic/glow bowling`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} offering cosmic/glow bowling`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/cosmic-bowling"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-cosmic-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/cosmic-bowling")}
                className="text-primary hover:text-primary/80"
                data-testid="button-back-to-cosmic"
              >
                <ArrowLeft className="w-4 h-4 mr-2" />
                All States
              </Button>
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-cosmic-title"
            >
              Cosmic Bowling in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see cosmic bowling venues. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search cosmic bowling venues in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-cosmic-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} cosmic bowling venues matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("cosmic_state_click", "navigation", state);
            setLocation(`/cosmic-bowling/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🌟"
          testIdPrefix="cosmic-state"
          description="Click to browse cosmic bowling"
          emptyStateTitle="No cosmic bowling locations found"
          emptyStateMessage="We're still building our database of cosmic bowling venues."
          emptyStateIcon="🌟"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
            ...
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  ...
                  setLocation(`/cosmic-bowling/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-cosmic-city-${city.slug}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse cosmic bowling
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with cosmic bowling found
            </h3>
            <p className="text-muted-foreground">
              No cosmic bowling found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🌟</div>
          <h3 className="text-lg font-semibold mb-2">
            No cosmic bowling venues found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No cosmic bowling found matching "${searchTerm}" in ${displayCity}, ${displayState}.`
              : `No cosmic bowling found in ${displayCity}, ${displayState}.`}
          </p>
        </div>
      )}
