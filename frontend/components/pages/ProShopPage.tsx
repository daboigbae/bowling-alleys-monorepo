...
export default function ProShop({ state: propState, city: propCity }: ProShopPageProps = {}) {
  ...
  // Set page title and meta description with pro shop focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Bowling Pro Shop in ${displayCity}, ${displayState} | Equipment & Services`;
      description = `Find bowling pro shops in ${displayCity}, ${displayState}. Discover bowling alleys with pro shop services including ball drilling, equipment sales, and expert fitting in your area.`;
    } else if (displayState) {
      title = `Bowling Pro Shop in ${displayState} | Equipment & Services by City`;
      description = `Browse bowling pro shops across ${displayState}. Find bowling alleys with pro shop services including ball drilling, equipment sales, and expert fitting in cities throughout the state.`;
    } else {
      title = "Bowling Pro Shop Near Me | Find Equipment & Services by State";
      description =
        "Discover bowling pro shops across the United States. Find bowling alleys with pro shop services including ball drilling, equipment sales, and expert fitting in your state and city.";
    }
    ...
    // Add structured data for pro shop organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 2,
            name: "Pro Shop",
            item: `${window.location.origin}/pro-shop`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Pro Shop`,
                    item: `${window.location.origin}/pro-shop/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Pro Shop`,
                    item: `${window.location.origin}/pro-shop/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Pro Shop in ${displayCity}, ${displayState}`
            : displayState
              ? `Pro Shop in ${displayState}`
              : "Pro Shop by State",
        description: description,
      },
    };

    // Remove existing structured data for pro shop
    const existingScript = document.querySelector(
      'script[type="application/ld+json"][data-proshop]',
    );
    if (existingScript) {
    ...
    script.setAttribute("data-proshop", "true");
    script.textContent = JSON.stringify(structuredData);
    document.head.appendChild(script);
  }, [displayState, displayCity, selectedState, selectedCity]);

  // Query for states list (only states with pro shop)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/proshop-states"],
    queryFn: getProShopStates,
  });

  // Query for venues in selected state (only venues with pro shop)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/proshop-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getProShopVenuesByState(displayState!)
        : Promise.resolve([]),
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-proshop-title"
            >
              Browse Bowling Pro Shops by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find bowling alleys with pro shop services including ball drilling, equipment sales, and expert fitting
              in your area. Select a state to see pro shop options
              organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/pro-shop/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-proshop-title"
            >
              Bowling Pro Shop in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby with pro shops`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} with pro shops`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/pro-shop"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-proshop-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/pro-shop")}
                className="text-primary hover:text-primary/80"
                data-testid="button-back-to-proshop"
              >
                <ArrowLeft className="w-4 h-4 mr-2" />
                All States
              </Button>
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-proshop-title"
            >
              Bowling Pro Shop in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see pro shop options. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search bowling pro shops in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-proshop-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} bowling pro shops matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("proshop_state_click", "navigation", state);
            setLocation(`/pro-shop/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🎳"
          testIdPrefix="proshop-state"
          description="Click to browse pro shops"
          emptyStateTitle="No bowling pro shops found"
          emptyStateMessage="We're still building our database of bowling alleys with pro shop services."
          emptyStateIcon="🎳"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
            ...
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  ...
                  setLocation(`/pro-shop/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-proshop-city-${city.slug}`}
              >
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with pro shops found
            </h3>
            <p className="text-muted-foreground">
              No bowling pro shops found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="space-y-8">
          {Object.entries(filteredVenuesByCity)
            .sort(([cityA], [cityB]) => {
              // Primary city (matching selected city) comes first
              if (cityA === displayCity && cityB !== displayCity) return -1;
              if (cityA !== displayCity && cityB === displayCity) return 1;
              ...
              <div key={city}>
                <h2 className="text-2xl font-semibold mb-4">
                  {city === displayCity ? (
                    ...
                    <>
                      <span className="text-muted-foreground">Nearby in </span>
                      {city}
                    </>
                  )}
                </h2>
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🎳</div>
          <h3 className="text-lg font-semibold mb-2">
            No bowling pro shops found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No pro shops found matching "${searchTerm}" in ${displayCity}, ${displayState}.`
              : `No pro shops found in ${displayCity}, ${displayState}.`}
          </p>
        </div>
      )}
