...
import {
  getDuckpinBowlingStates,
  getDuckpinBowlingVenuesByState,
  Venue,
} from "@/lib/firestore";
...
interface DuckpinBowlingPageProps { state?: string; city?: string; }
export default function LaserTag({ state: propState, city: propCity }: DuckpinBowlingPageProps = {}) {
  const [cityMatch, cityParams] = useRoute("/duckpin-bowling/:state/:city");
  const [stateMatch, stateParams] = useRoute("/duckpin-bowling/:state");
  const [baseMatch] = useRoute("/duckpin-bowling");
  ...
  // Set page title and meta description with duckpin bowling focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Bowling & Duckpin Bowling in ${displayCity}, ${displayState} | Fun Combined`;
      description = `Find bowling alleys with duckpin bowling in ${displayCity}, ${displayState}. Discover venues offering both bowling and duckpin bowling for the ultimate entertainment experience.`;
    } else if (displayState) {
      title = `Bowling & Duckpin Bowling in ${displayState} | Combined Fun by City`;
      description = `Browse bowling alleys with duckpin bowling across ${displayState}. Find venues offering both bowling and duckpin bowling entertainment in cities throughout the state.`;
    } else {
      title = "Bowling & Duckpin Bowling Near Me | Find Combined Entertainment by State";
      description =
        "Discover bowling alleys with duckpin bowling across the United States. Find venues offering both bowling and duckpin bowling for maximum fun in your state and city.";
    }
    ...
    // Add structured data for duckpin bowling organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            item: `${window.location.origin}/duckpin-bowling`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Duckpin Bowling`,
                    item: `${window.location.origin}/duckpin-bowling/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Duckpin Bowling`,
                    item: `${window.location.origin}/duckpin-bowling/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Duckpin Bowling in ${displayCity}, ${displayState}`
            : displayState
              ? `Duckpin Bowling in ${displayState}`
              : "Duckpin Bowling by State",
        description: description,
      },
    };

    // Remove existing structured data for duckpin bowling
    const existingScript = document.querySelector(
      'script[type="application/ld+json"][data-lasertag]',
    );
    if (existingScript) {
    ...
    script.setAttribute("data-lasertag", "true");
    script.textContent = JSON.stringify(structuredData);
    document.head.appendChild(script);
  }, [displayState, displayCity, selectedState, selectedCity]);

  // Query for states list (only states with duckpin bowling)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/lasertag-states"],
    queryFn: getDuckpinBowlingStates,
  });

  // Query for venues in selected state (only venues with duckpin bowling)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/lasertag-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getDuckpinBowlingVenuesByState(displayState!)
        : Promise.resolve([]),
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-lasertag-title"
            >
              Browse Bowling & Duckpin Bowling by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find bowling alleys with duckpin bowling for the ultimate entertainment combo. Perfect for parties, group events, or just having a blast with friends. Select a state to see venues
              organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              ...
              <Link
                href={`/duckpin-bowling/${selectedState}`}
                className="text-primary hover:text-primary/80 underline"
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/duckpin-bowling/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-lasertag-title"
            >
              Bowling & Duckpin Bowling in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby with duckpin bowling`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} with duckpin bowling`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/duckpin-bowling"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-lasertag-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/duckpin-bowling")}
                className="text-primary hover:text-primary/80"
                data-testid="button-back-to-lasertag"
              >
                <ArrowLeft className="w-4 h-4 mr-2" />
                All States
              </Button>
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-lasertag-title"
            >
              Bowling & Duckpin Bowling in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see duckpin bowling options. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search bowling & duckpin bowling venues in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-lasertag-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} venues with duckpin bowling matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("lasertag_state_click", "navigation", state);
            setLocation(`/duckpin-bowling/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🦆"
          testIdPrefix="lasertag-state"
          description="Click to browse duckpin bowling venues"
          emptyStateTitle="No bowling & duckpin bowling venues found"
          emptyStateMessage="We're still building our database of bowling alleys with duckpin bowling."
          emptyStateIcon="🦆"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
            ...
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  ...
                  setLocation(`/duckpin-bowling/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-lasertag-city-${city.slug}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse duckpin bowling venues
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with duckpin bowling found
            </h3>
            <p className="text-muted-foreground">
              No bowling & duckpin bowling venues found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🦆</div>
          <h3 className="text-lg font-semibold mb-2">
            No duckpin bowling venues found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No duckpin bowling venues matching "${searchTerm}" in ${displayCity}`
              : `No bowling & duckpin bowling venues found in ${displayCity}`}
          </p>
        </div>
      )}
