...
interface BowlingBirthdayPartyPageProps { state?: string; city?: string; }
export default function BowlingBirthdayParty({ state: propState, city: propCity }: BowlingBirthdayPartyPageProps = {}) {
  const [cityMatch, cityParams] = useRoute("/bowling-birthday-party/:state/:city");
  const [stateMatch, stateParams] = useRoute("/bowling-birthday-party/:state");
  const [baseMatch] = useRoute("/bowling-birthday-party");
  ...
  // Set page title and meta description with birthday party focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Bowling Birthday Parties in ${displayCity}, ${displayState} | Kids Birthday Party Venues`;
      description = `Find bowling alleys perfect for birthday parties in ${displayCity}, ${displayState}. Discover venues offering kids parties, group packages, and birthday party services in your area.`;
    } else if (displayState) {
      title = `Bowling Birthday Parties in ${displayState} | Party Venues by City`;
      description = `Browse bowling birthday party venues across ${displayState}. Find bowling alleys offering kids parties, group packages, and birthday party services in cities throughout the state.`;
    } else {
      title = "Bowling Birthday Parties Near Me | Find Party Venues by State";
      description =
        "Discover bowling birthday party venues across the United States. Find bowling alleys offering kids parties, group packages, and birthday party services in your state and city.";
    }
    ...
    // Add structured data for birthday party organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            "@type": "ListItem",
            position: 2,
            name: "Bowling Birthday Parties",
            item: `${window.location.origin}/bowling-birthday-party`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Birthday Parties`,
                    item: `${window.location.origin}/bowling-birthday-party/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Birthday Parties`,
                    item: `${window.location.origin}/bowling-birthday-party/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Birthday Parties in ${displayCity}, ${displayState}`
            : displayState
              ? `Birthday Parties in ${displayState}`
              : "Bowling Birthday Parties by State",
        description: description,
      },
    };

    // Remove existing structured data for birthday parties
    const existingScript = document.querySelector(
      'script[type="application/ld+json"][data-party]',
    );
    if (existingScript) {
    ...
    script.setAttribute("data-party", "true");
    script.textContent = JSON.stringify(structuredData);
    document.head.appendChild(script);
  }, [displayState, displayCity, selectedState, selectedCity]);

  // Query for states list (only states with birthday party venues)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/party-states"],
    queryFn: getPartyStates,
  });

  // Query for venues in selected state (only venues with birthday party support)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/party-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getPartyVenuesByState(displayState!)
        : Promise.resolve([]),
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-party-title"
            >
              Browse Bowling Birthday Parties by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find bowling alleys perfect for birthday parties in your area. Select a state to see venues offering kids parties and birthday packages organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              ...
              <Link
                href={`/bowling-birthday-party/${selectedState}`}
                className="text-primary hover:text-primary/80 underline"
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/bowling-birthday-party/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-party-title"
            >
              Bowling Birthday Parties in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'venue' : 'venues'} in ${displayCity} + ${nearbyVenues} nearby offering birthday party packages`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'venue' : 'venues'} offering birthday party packages`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/bowling-birthday-party"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-party-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/bowling-birthday-party")}
                className="text-primary hover:text-primary/80"
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-party-title"
            >
              Bowling Birthday Parties in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see birthday party venues. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search birthday party venues in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-party-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} birthday party venues matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("party_state_click", "navigation", state);
            setLocation(`/bowling-birthday-party/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🎉"
          testIdPrefix="party-state"
          description="Click to browse birthday parties"
          emptyStateTitle="No birthday party venues found"
          emptyStateMessage="We're still building our database of birthday party venues."
          emptyStateIcon="🎉"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
            ...
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  ...
                  setLocation(`/bowling-birthday-party/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-party-city-${city.slug}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse birthday parties
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with birthday party venues found
            </h3>
            <p className="text-muted-foreground">
              No birthday party venues found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🎉</div>
          <h3 className="text-lg font-semibold mb-2">
            No birthday party venues found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No birthday party venues found matching "${searchTerm}" in ${displayCity}, ${displayState}.`
              : `No birthday party venues found in ${displayCity}, ${displayState}.`}
          </p>
        </div>
      )}
