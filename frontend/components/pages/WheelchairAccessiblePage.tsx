...
import {
  getWheelchairAccessibleStates,
  getWheelchairAccessibleVenuesByState,
  Venue,
} from "@/lib/firestore";
...
interface WheelchairAccessiblePageProps { state?: string; city?: string; }
export default function WheelchairAccessible({ state: propState, city: propCity }: WheelchairAccessiblePageProps = {}) {
  const [cityMatch, cityParams] = useRoute("/wheelchair-accessible/:state/:city");
  ...
  // Set page title and meta description with wheelchair accessible focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Wheelchair Accessible Bowling Alleys in ${displayCity}, ${displayState} | ADA Compliant`;
      description = `Find wheelchair accessible bowling alleys in ${displayCity}, ${displayState}. Discover ADA-compliant venues with accessible lanes, ramps, and facilities for everyone.`;
    } else if (displayState) {
      title = `Wheelchair Accessible Bowling in ${displayState} | ADA Compliant by City`;
      description = `Browse wheelchair accessible bowling alleys across ${displayState}. Find ADA-compliant venues with accessible facilities in cities throughout the state.`;
    } else {
      title = "Wheelchair Accessible Bowling Alleys Near Me | Find ADA Compliant Venues by State";
      description =
        "Discover wheelchair accessible bowling alleys across the United States. Find ADA-compliant venues with accessible lanes, ramps, and facilities in your state and city.";
    }
    ...
    // Add structured data for wheelchair accessible organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            item: `${window.location.origin}/wheelchair-accessible`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Wheelchair Accessible`,
                    item: `${window.location.origin}/wheelchair-accessible/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Wheelchair Accessible`,
                    item: `${window.location.origin}/wheelchair-accessible/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Wheelchair Accessible in ${displayCity}, ${displayState}`
            : displayState
              ? `Wheelchair Accessible in ${displayState}`
              : "Wheelchair Accessible by State",
        description: description,
      },
    };

    // Remove existing structured data for wheelchair accessible
    const existingScript = document.querySelector(
      'script[type="application/ld+json"][data-wheelchair]',
    );
    if (existingScript) {
    ...
    script.setAttribute("data-wheelchair", "true");
    script.textContent = JSON.stringify(structuredData);
    document.head.appendChild(script);
  }, [displayState, displayCity, selectedState, selectedCity]);

  // Query for states list (only states with wheelchair accessible)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/wheelchair-states"],
    queryFn: getWheelchairAccessibleStates,
  });

  // Query for venues in selected state (only venues with wheelchair accessible)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/wheelchair-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getWheelchairAccessibleVenuesByState(displayState!)
        : Promise.resolve([]),
  ...
  const handleVenueClick = (venueId: string) => {
    // Store wheelchair accessible back path for the back button
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-wheelchair-title"
            >
              Browse Wheelchair Accessible Bowling Alleys by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find ADA-compliant bowling alleys with wheelchair accessibility features including ramps, accessible lanes, and facilities. Perfect for everyone to enjoy bowling. Select a state to see accessible venues
              organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              ...
              <Link
                href={`/wheelchair-accessible/${selectedState}`}
                className="text-primary hover:text-primary/80 underline"
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/wheelchair-accessible/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-wheelchair-title"
            >
              Wheelchair Accessible Bowling Alleys in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} accessible ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby`;
                }
                return `${totalVenues} wheelchair accessible bowling ${totalVenues === 1 ? 'alley' : 'alleys'}`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/wheelchair-accessible"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-wheelchair-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/wheelchair-accessible")}
                className="text-primary hover:text-primary/80"
                data-testid="button-back-to-wheelchair"
              >
                <ArrowLeft className="w-4 h-4 mr-2" />
                All States
              </Button>
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-wheelchair-title"
            >
              Wheelchair Accessible Bowling Alleys in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see wheelchair accessible bowling alleys. {stateVenues.length}{" "}
              accessible bowling {stateVenues.length === 1 ? "venue" : "venues"} across{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search wheelchair accessible bowling alleys in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-wheelchair-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} venues with wheelchair accessible matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("wheelchair_state_click", "navigation", state);
            setLocation(`/wheelchair-accessible/${state.replace(/\s+/g, "-")}`);
          }}
          icon="♿"
          testIdPrefix="wheelchair-state"
          description="Click to browse wheelchair accessible venues"
          emptyStateTitle="No wheelchair accessible bowling alleys found"
          emptyStateMessage="We're still building our database of ADA-compliant bowling alleys."
          emptyStateIcon="♿"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  trackEvent(
                    "wheelchair_city_click",
                    "navigation",
                    `${city.name}, ${displayState}`,
                  );
                  setLocation(`/wheelchair-accessible/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-wheelchair-city-${city.slug}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse wheelchair accessible venues
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with accessible bowling found
            </h3>
            <p className="text-muted-foreground">
              No wheelchair accessible bowling alleys found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="space-y-8">
          {Object.entries(filteredVenuesByCity)
            .sort(([cityA], [cityB]) => {
              // Sort so that the selected city is always first
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">♿</div>
          <h3 className="text-lg font-semibold mb-2">
            No wheelchair accessible bowling alleys found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No accessible bowling alleys matching "${searchTerm}" in ${displayCity}`
              : `No wheelchair accessible bowling alleys found in ${displayCity}`}
          </p>
        </div>
      )}
