...
export default function ArcadeBowling({ state: propState, city: propCity }: ArcadeBowlingPageProps = {}) {
  ...
  // Set page title and meta description with arcade focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Arcade Bowling in ${displayCity}, ${displayState} | Find Bowling Alleys with Arcades`;
      description = `Discover arcade bowling in ${displayCity}, ${displayState}. Find bowling alleys with arcade games, redemption prizes, and family entertainment centers in your area.`;
    } else if (displayState) {
      title = `Arcade Bowling in ${displayState} | Bowling Alleys with Arcades by City`;
      description = `Browse arcade bowling across ${displayState}. Find bowling alleys with arcade games, redemption prizes, and family entertainment centers in cities throughout the state.`;
    } else {
      title = "Arcade Bowling Near Me | Find Bowling Alleys with Arcades by State";
      description =
        "Discover arcade bowling across the United States. Find bowling alleys with arcade games, redemption prizes, and family entertainment centers in your state and city.";
    }
    ...
    // Add structured data for arcade bowling organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 2,
            name: "Arcade Bowling",
            item: `${window.location.origin}/arcade-bowling`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Arcade Bowling`,
                    item: `${window.location.origin}/arcade-bowling/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Arcade Bowling`,
                    item: `${window.location.origin}/arcade-bowling/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Arcade Bowling in ${displayCity}, ${displayState}`
            : displayState
              ? `Arcade Bowling in ${displayState}`
              : "Arcade Bowling by State",
        description: description,
      },
    };

    // Remove existing structured data for arcade bowling
  ...
  // Query for states list (only states with arcade bowling)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/arcade-states"],
    queryFn: getArcadeStates,
  });

  // Query for venues in selected state (only venues with arcade)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/arcade-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getArcadeVenuesByState(displayState!)
        : Promise.resolve([]),
  ...
  const handleVenueClick = (venueId: string) => {
    // Store arcade bowling back path for the back button
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-arcade-title"
            >
              Browse Arcade Bowling by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find bowling alleys with arcade games and entertainment centers
              in your area. Select a state to see arcade bowling opportunities
              organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              ...
              <Link
                href={`/arcade-bowling/${selectedState}`}
                className="text-primary hover:text-primary/80 underline"
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/arcade-bowling/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-arcade-title"
            >
              Arcade Bowling in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby with arcade games`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} with arcade games`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/arcade-bowling"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-arcade-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/arcade-bowling")}
                className="text-primary hover:text-primary/80"
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-arcade-title"
            >
              Arcade Bowling in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see arcade bowling venues. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search arcade bowling venues in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-arcade-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} arcade bowling venues matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("arcade_state_click", "navigation", state);
            setLocation(`/arcade-bowling/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🎮"
          testIdPrefix="arcade-state"
          description="Click to browse arcade bowling"
          emptyStateTitle="No arcade bowling locations found"
          emptyStateMessage="We're still building our database of arcade bowling venues."
          emptyStateIcon="🎮"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
            ...
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  ...
                  setLocation(`/arcade-bowling/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-arcade-city-${city.slug}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse arcade bowling
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with arcade bowling found
            </h3>
            <p className="text-muted-foreground">
              No arcade bowling found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🎮</div>
          <h3 className="text-lg font-semibold mb-2">
            No arcade bowling venues found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No arcade bowling found matching "${searchTerm}" in ${displayCity}, ${displayState}.`
              : `No arcade bowling found in ${displayCity}, ${displayState}.`}
          </p>
        </div>
      )}
