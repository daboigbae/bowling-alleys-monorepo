...
export default function BowlingBilliards({ state: propState, city: propCity }: BowlingBilliardsPageProps = {}) {
  ...
  // Set page title and meta description with billiards focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Bowling Alleys with Pool Tables in ${displayCity}, ${displayState} | Billiards & Bowling`;
      description = `Discover bowling alleys with pool tables in ${displayCity}, ${displayState}. Find venues with both bowling lanes and billiards tables for a complete entertainment experience.`;
    } else if (displayState) {
      title = `Bowling & Billiards in ${displayState} | Bowling Alleys with Pool Tables by City`;
      description = `Browse bowling alleys with billiards across ${displayState}. Find venues with pool tables and bowling lanes in cities throughout the state.`;
    } else {
      title = "Bowling & Billiards Near Me | Find Bowling Alleys with Pool Tables by State";
      description =
        "Discover bowling alleys with pool tables across the United States. Find venues offering both bowling and billiards in your state and city.";
    }
    ...
    // Add structured data for billiards bowling organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            item: `${window.location.origin}/bowling-billiards`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Bowling & Billiards`,
                    item: `${window.location.origin}/bowling-billiards/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Bowling & Billiards`,
                    item: `${window.location.origin}/bowling-billiards/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Bowling & Billiards in ${displayCity}, ${displayState}`
            : displayState
              ? `Bowling & Billiards in ${displayState}`
              : "Bowling & Billiards by State",
        description: description,
      },
    };

    // Remove existing structured data for billiards bowling
  ...
  // Query for states list (only states with bowling & billiards)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/billiards-states"],
    queryFn: getBilliardsStates,
  });

  // Query for venues in selected state (only venues with billiards/pool)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/billiards-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getBilliardsVenuesByState(displayState!)
        : Promise.resolve([]),
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-billiards-title"
            >
              Browse Bowling & Billiards by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find bowling alleys with pool tables in your area. 
              Select a state to see bowling and billiards venues organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              ...
              <Link
                href={`/bowling-billiards/${selectedState}`}
                className="text-primary hover:text-primary/80 underline"
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/bowling-billiards/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-billiards-title"
            >
              Bowling & Billiards in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby with pool tables`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} with billiards/pool tables`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/bowling-billiards"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-billiards-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/bowling-billiards")}
                className="text-primary hover:text-primary/80"
                data-testid="button-back-to-billiards"
              >
                <ArrowLeft className="w-4 h-4 mr-2" />
                All States
              </Button>
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-billiards-title"
            >
              Bowling & Billiards in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see bowling alleys with pool tables. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search bowling & billiards venues in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-billiards-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} bowling & billiards venues matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("billiards_state_click", "navigation", state);
            setLocation(`/bowling-billiards/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🎱"
          testIdPrefix="billiards-state"
          description="Click to browse bowling & billiards"
          emptyStateTitle="No bowling & billiards locations found"
          emptyStateMessage="We're still building our database of bowling alleys with pool tables."
          emptyStateIcon="🎱"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  trackEvent(
                    "billiards_city_click",
                    "navigation",
                    `${city.name}, ${displayState}`,
                  );
                  setLocation(`/bowling-billiards/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-billiards-city-${city.slug}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse bowling & billiards
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with bowling & billiards found
            </h3>
            <p className="text-muted-foreground">
              No bowling alleys with pool tables found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🎱</div>
          <h3 className="text-lg font-semibold mb-2">
            No billiards bowling venues found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No billiards bowling found matching "${searchTerm}" in ${displayCity}, ${displayState}.`
              : `No billiards bowling found in ${displayCity}, ${displayState}.`}
          </p>
        </div>
      )}
