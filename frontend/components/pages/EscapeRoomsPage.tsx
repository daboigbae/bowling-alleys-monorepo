...
import {
  getEscapeRoomsStates,
  getEscapeRoomsVenuesByState,
  Venue,
} from "@/lib/firestore";
...
export default function EscapeRooms({ state: propState, city: propCity }: EscapeRoomsPageProps = {}) {
  ...
  // Set page title and meta description with escape rooms focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Bowling Alleys with Escape Rooms in ${displayCity}, ${displayState} | Fun Combined`;
      description = `Find bowling alleys with escape rooms in ${displayCity}, ${displayState}. Discover venues offering both bowling and escape room adventures for the ultimate entertainment experience.`;
    } else if (displayState) {
      title = `Bowling Alleys with Escape Rooms in ${displayState} | Entertainment Combined`;
      description = `Browse bowling alleys with escape rooms across ${displayState}. Find venues offering both bowling and escape room adventures in cities throughout the state.`;
    } else {
      title = "Bowling Alleys with Escape Rooms Near Me | Find Entertainment by State";
      description =
        "Discover bowling alleys with escape rooms across the United States. Find venues offering both bowling and escape room experiences for maximum fun in your state and city.";
    }
    ...
    // Add structured data for escape rooms organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 2,
            name: "Escape Rooms",
            item: `${window.location.origin}/escape-rooms`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Escape Rooms`,
                    item: `${window.location.origin}/escape-rooms/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Escape Rooms`,
                    item: `${window.location.origin}/escape-rooms/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Escape Rooms in ${displayCity}, ${displayState}`
            : displayState
              ? `Escape Rooms in ${displayState}`
              : "Escape Rooms by State",
        description: description,
      },
    };

    // Remove existing structured data for escape rooms
    const existingScript = document.querySelector(
      'script[type="application/ld+json"][data-escaperooms]',
    );
    if (existingScript) {
    ...
    script.setAttribute("data-escaperooms", "true");
    script.textContent = JSON.stringify(structuredData);
    document.head.appendChild(script);
  }, [displayState, displayCity, selectedState, selectedCity]);

  // Query for states list (only states with escape rooms)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/escaperooms-states"],
    queryFn: getEscapeRoomsStates,
  });

  // Query for venues in selected state (only venues with escape rooms)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/escaperooms-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getEscapeRoomsVenuesByState(displayState!)
        : Promise.resolve([]),
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-escaperooms-title"
            >
              Browse Bowling Alleys with Escape Rooms by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find bowling alleys with escape rooms for the ultimate entertainment combo. Perfect for parties, group events, team building, or just having a blast with friends. Select a state to see venues
              organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              ...
              <Link
                href={`/escape-rooms/${selectedState}`}
                className="text-primary hover:text-primary/80 underline"
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/escape-rooms/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-escaperooms-title"
            >
              Bowling Alleys with Escape Rooms in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby with escape rooms`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} with escape rooms`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/escape-rooms"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-escaperooms-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/escape-rooms")}
                className="text-primary hover:text-primary/80"
                data-testid="button-back-to-escaperooms"
              >
                <ArrowLeft className="w-4 h-4 mr-2" />
                All States
              </Button>
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-escaperooms-title"
            >
              Bowling Alleys with Escape Rooms in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see escape room options. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search bowling alleys with escape rooms in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-escaperooms-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} venues with escape rooms matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("escaperooms_state_click", "navigation", state);
            setLocation(`/escape-rooms/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🔐"
          testIdPrefix="escaperooms-state"
          description="Click to browse bowling alleys with escape rooms"
          emptyStateTitle="No bowling alleys with escape rooms found"
          emptyStateMessage="We're still building our database of bowling alleys with escape rooms."
          emptyStateIcon="🔐"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  trackEvent(
                    "escaperooms_city_click",
                    "navigation",
                    `${city.name}, ${displayState}`,
                  );
                  setLocation(`/escape-rooms/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-escaperooms-city-${city.slug}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse bowling alleys with escape rooms
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with escape rooms found
            </h3>
            <p className="text-muted-foreground">
              No bowling alleys with escape rooms found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="space-y-8">
          {/* Primary city venues first */}
          {filteredVenuesByCity[displayCity!] && (
            <div>
              <h2 className="text-xl font-semibold mb-4">
                In {displayCity}
              </h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredVenuesByCity[displayCity!].map((venue) => (
                  <VenueCard
                    key={venue.id}
                    venue={venue}
        ...
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            {searchTerm
              ? `No bowling alleys with escape rooms matching "${searchTerm}" found in ${displayCity}.`
              : `No bowling alleys with escape rooms found in ${displayCity}.`}
          </p>
        </div>
      )}
