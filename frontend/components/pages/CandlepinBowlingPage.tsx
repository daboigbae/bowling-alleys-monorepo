...
import {
  getCandlepinBowlingStates,
  getCandlepinBowlingVenuesByState,
  Venue,
} from "@/lib/firestore";
...
export default function CandlepinBowling({ state: propState, city: propCity }: CandlepinBowlingPageProps = {}) {
  ...
  // Set page title and meta description with candlepin bowling focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Candlepin Bowling in ${displayCity}, ${displayState} | Traditional New England Bowling`;
      description = `Find candlepin bowling alleys in ${displayCity}, ${displayState}. Discover venues offering traditional New England candlepin bowling with smaller balls and unique gameplay.`;
    } else if (displayState) {
      title = `Candlepin Bowling in ${displayState} | Traditional Bowling by City`;
      description = `Browse candlepin bowling alleys across ${displayState}. Find venues offering traditional New England candlepin bowling in cities throughout the state.`;
    } else {
      title = "Candlepin Bowling Near Me | Find Traditional New England Bowling by State";
      description =
        "Discover candlepin bowling alleys across the United States. Find venues offering traditional New England candlepin bowling with smaller balls and pins in your state and city.";
    }
    ...
    // Add structured data for candlepin bowling organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            item: `${window.location.origin}/candlepin-bowling`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Candlepin Bowling`,
                    item: `${window.location.origin}/candlepin-bowling/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Candlepin Bowling`,
                    item: `${window.location.origin}/candlepin-bowling/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Candlepin Bowling in ${displayCity}, ${displayState}`
            : displayState
              ? `Candlepin Bowling in ${displayState}`
              : "Candlepin Bowling by State",
        description: description,
      },
    };

    // Remove existing structured data for candlepin bowling
    const existingScript = document.querySelector(
      'script[type="application/ld+json"][data-candlepin]',
    );
    if (existingScript) {
    ...
    script.setAttribute("data-candlepin", "true");
    script.textContent = JSON.stringify(structuredData);
    document.head.appendChild(script);
  }, [displayState, displayCity, selectedState, selectedCity]);

  // Query for states list (only states with candlepin bowling)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/candlepin-states"],
    queryFn: getCandlepinBowlingStates,
  });

  // Query for venues in selected state (only venues with candlepin bowling)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/candlepin-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getCandlepinBowlingVenuesByState(displayState!)
        : Promise.resolve([]),
  ...
  const handleVenueClick = (venueId: string) => {
    // Store candlepin bowling back path for the back button
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!selectedState ? (
          <>
            <h1
              ...
              data-testid="text-candlepin-title"
            >
              Browse Candlepin Bowling by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find candlepin bowling alleys offering traditional New England bowling with smaller balls and unique gameplay. Perfect for experiencing classic bowling heritage. Select a state to see venues
              organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              ...
              <Link
                href={`/candlepin-bowling/${selectedState}`}
                className="text-primary hover:text-primary/80 underline"
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/candlepin-bowling/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-candlepin-title"
            >
              Candlepin Bowling in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby with candlepin bowling`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} with candlepin bowling`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/candlepin-bowling"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-candlepin-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/candlepin-bowling")}
                className="text-primary hover:text-primary/80"
                data-testid="button-back-to-candlepin"
              >
                <ArrowLeft className="w-4 h-4 mr-2" />
                All States
              </Button>
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-candlepin-title"
            >
              Candlepin Bowling in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see candlepin bowling options. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search candlepin bowling venues in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-candlepin-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} venues with candlepin bowling matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("candlepin_state_click", "navigation", state);
            setLocation(`/candlepin-bowling/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🎳"
          testIdPrefix="candlepin-state"
          description="Click to browse candlepin bowling venues"
          emptyStateTitle="No candlepin bowling venues found"
          emptyStateMessage="We're still building our database of candlepin bowling alleys."
          emptyStateIcon="🎳"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  trackEvent(
                    "candlepin_city_click",
                    "navigation",
                    `${city.name}, ${displayState}`,
                  );
                  setLocation(`/candlepin-bowling/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-candlepin-city-${city.slug}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse candlepin bowling venues
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with candlepin bowling found
            </h3>
            <p className="text-muted-foreground">
              No candlepin bowling venues found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🎳</div>
          <h3 className="text-lg font-semibold mb-2">
            No candlepin bowling venues found
          </h3>
          <p className="text-muted-foreground mb-4">
            {searchTerm
              ? `No venues match "${searchTerm}" in ${displayCity}.`
              : `No candlepin bowling venues found in ${displayCity}.`}
          </p>
          <p className="text-sm text-muted-foreground mb-6">
            Know a candlepin bowling alley that should be listed? We'd love to
            hear from you!
          </p>
          <Button
            variant="outline"
            onClick={() => (window.location.href = "mailto:support@bowlingalleys.io")}
