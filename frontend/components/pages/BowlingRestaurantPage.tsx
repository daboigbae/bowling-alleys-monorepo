...
interface BowlingRestaurantPageProps { state?: string; city?: string; }
export default function BowlingRestaurant({ state: propState, city: propCity }: BowlingRestaurantPageProps = {}) {
  const [cityMatch, cityParams] = useRoute("/bowling-restaurant/:state/:city");
  const [stateMatch, stateParams] = useRoute("/bowling-restaurant/:state");
  const [baseMatch] = useRoute("/bowling-restaurant");
  ...
  // Set page title and meta description with restaurant focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Bowling Alleys with Restaurants in ${displayCity}, ${displayState} | Food & Drinks`;
      description = `Discover bowling alleys with restaurants in ${displayCity}, ${displayState}. Find bowling centers with full-service dining, snack bars, and great food options in your area.`;
    } else if (displayState) {
      title = `Bowling Alleys with Restaurants in ${displayState} | Food & Dining by City`;
      description = `Browse bowling alleys with restaurants across ${displayState}. Find bowling centers with full-service dining, snack bars, and great food options in cities throughout the state.`;
    } else {
      title = "Bowling Alleys with Restaurants Near Me | Find Food & Dining by State";
      description =
        "Discover bowling alleys with restaurants across the United States. Find bowling centers with full-service dining, snack bars, and great food options in your state and city.";
    }
    ...
    // Add structured data for restaurant bowling organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            "@type": "ListItem",
            position: 2,
            name: "Bowling Restaurants",
            item: `${window.location.origin}/bowling-restaurant`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Bowling Restaurants`,
                    item: `${window.location.origin}/bowling-restaurant/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Bowling Restaurants`,
                    item: `${window.location.origin}/bowling-restaurant/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Bowling Restaurants in ${displayCity}, ${displayState}`
            : displayState
              ? `Bowling Restaurants in ${displayState}`
              : "Bowling Restaurants by State",
        description: description,
      },
    };

    // Remove existing structured data for restaurant bowling
    ...
    script.setAttribute("data-restaurant", "true");
    script.textContent = JSON.stringify(structuredData);
    document.head.appendChild(script);
  }, [displayState, displayCity, selectedState, selectedCity]);

  // Query for states list (only states with restaurant bowling)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/restaurant-states"],
    queryFn: getRestaurantStates,
  });

  // Query for venues in selected state (only venues with restaurant)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/restaurant-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getRestaurantVenuesByState(displayState!)
        : Promise.resolve([]),
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-8">
        {!selectedState ? (
          <>
            ...
            >
              Browse Bowling Alleys with Restaurants by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find bowling alleys with full-service restaurants and great food
              in your area. Select a state to see dining options
              organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              ...
              <Link
                href={`/bowling-restaurant/${selectedState}`}
                className="text-primary hover:text-primary/80 underline"
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/bowling-restaurant/${selectedState}`)}
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-city-restaurant-title"
            >
              Bowling Alleys with Restaurants in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby with restaurants`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} with restaurants`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            <nav className="flex items-center gap-2 mb-4 text-sm">
              <Link
                href="/bowling-restaurant"
                className="text-primary hover:text-primary/80 underline"
                data-testid="breadcrumb-restaurant-home"
              >
                All States
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation("/bowling-restaurant")}
                className="text-primary hover:text-primary/80"
            ...
            <h1
              className="text-3xl font-bold text-foreground mb-4"
              data-testid="text-state-restaurant-title"
            >
              Bowling Alleys with Restaurants in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see dining options. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search bowling alleys with restaurants in this city..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10"
              data-testid="input-restaurant-venue-search"
            />
            {searchTerm && (
              <Button
                variant="ghost"
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} bowling alleys with restaurants matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            trackEvent("restaurant_state_click", "navigation", state);
            setLocation(`/bowling-restaurant/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🍔"
          testIdPrefix="restaurant-state"
          description="Click to browse restaurants"
          emptyStateTitle="No bowling alleys with restaurants found"
          emptyStateMessage="We're still building our database of bowling alleys with dining options."
          emptyStateIcon="🍔"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  trackEvent(
                    "restaurant_city_click",
                    "navigation",
                    `${city.name}, ${displayState}`,
                  );
                  setLocation(`/bowling-restaurant/${selectedState}/${city.slug}`);
                }}
                data-testid={`card-restaurant-city-${city.slug}`}
              >
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with bowling restaurants found
            </h3>
            <p className="text-muted-foreground">
              No bowling alleys with restaurants found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🍔</div>
          <h3 className="text-lg font-semibold mb-2">
            No bowling alleys with restaurants found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No bowling alleys with restaurants found matching "${searchTerm}" in ${displayCity}, ${displayState}.`
              : `No bowling alleys with restaurants found in ${displayCity}, ${displayState}.`}
          </p>
        </div>
      )}
