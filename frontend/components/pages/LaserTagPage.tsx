...
export default function LaserTag({ state: propState, city: propCity }: LaserTagPageProps = {}) {
  ...
  // Set page title and meta description with laser tag focus
  useEffect(() => {
    let title, description;

    if (displayCity && displayState) {
      title = `Bowling & Laser Tag in ${displayCity}, ${displayState} | Fun Combined`;
      description = `Find bowling alleys with laser tag in ${displayCity}, ${displayState}. Discover venues offering both bowling and laser tag for the ultimate entertainment experience.`;
    } else if (displayState) {
      title = `Bowling & Laser Tag in ${displayState} | Combined Fun by City`;
      description = `Browse bowling alleys with laser tag across ${displayState}. Find venues offering both bowling and laser tag entertainment in cities throughout the state.`;
    } else {
      title = "Bowling & Laser Tag Near Me | Find Combined Entertainment by State";
      description =
        "Discover bowling alleys with laser tag across the United States. Find venues offering both bowling and laser tag for maximum fun in your state and city.";
    }
    ...
    // Add structured data for laser tag organization
    const structuredData = {
      ...
      breadcrumb: {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 2,
            name: "Laser Tag",
            item: `${window.location.origin}/laser-tag`,
          },
        ]
          .concat(
            displayState
              ? [
                  {
                    "@type": "ListItem",
                    position: 3,
                    name: `${displayState} Laser Tag`,
                    item: `${window.location.origin}/laser-tag/${selectedState}`,
                  },
                ]
              : [],
          )
              ...
              ? [
                  {
                    "@type": "ListItem",
                    position: 4,
                    name: `${displayCity} Laser Tag`,
                    item: `${window.location.origin}/laser-tag/${selectedState}/${selectedCity}`,
                  },
                ]
              : [],
          ),
      ...
      mainEntity: {
        ...
        name:
          displayCity && displayState
            ? `Laser Tag in ${displayCity}, ${displayState}`
            : displayState
              ? `Laser Tag in ${displayState}`
              : "Laser Tag by State",
        description: description,
      },
    };

    // Remove existing structured data for laser tag
  ...
  // Query for states list (only states with laser tag)
  const { data: states = [], isLoading: statesLoading } = useQuery({
    queryKey: ["/api/lasertag-states"],
    queryFn: getLaserTagStates,
  });

  // Query for venues in selected state (only venues with laser tag)
  const { data: stateVenues = [], isLoading: venuesLoading } = useQuery({
    queryKey: ["/api/lasertag-venues-by-state", selectedState],
    queryFn: () =>
      selectedState
        ? getLaserTagVenuesByState(displayState!)
        : Promise.resolve([]),
  ...
  const handleVenueClick = (venueId: string) => {
    // Store laser tag back path for the back button
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-8">
        {!selectedState ? (
          <>
            ...
            >
              Browse Bowling & Laser Tag by Location
            </h1>
            <p className="text-muted-foreground text-lg">
              Find bowling alleys with laser tag for the ultimate entertainment combo. Perfect for parties, group events, or just having a blast with friends. Select a state to see venues
              organized by city.
            </p>
          </>
        ) : displayCity ? (
          <>
            ...
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="ghost"
                onClick={() => setLocation(`/laser-tag/${selectedState}`)}
            ...
            >
              Bowling & Laser Tag in {displayCity}, {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              {(() => {
                ...
                if (nearbyVenues > 0) {
                  return `${cityVenues} ${cityVenues === 1 ? 'alley' : 'alleys'} in ${displayCity} + ${nearbyVenues} nearby with laser tag`;
                }
                return `${totalVenues} bowling ${totalVenues === 1 ? 'alley' : 'alleys'} with laser tag`;
              })()}
            </p>
          </>
        ) : (
          <>
            ...
            >
              Bowling & Laser Tag in {displayState}
            </h1>
            <p className="text-muted-foreground text-lg">
              Select a city to see laser tag options. {stateVenues.length}{" "}
      ...
      {selectedCity && (
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search bowling & laser tag venues in this city..."
              value={searchTerm}
          ...
          {searchTerm && (
            <p className="text-sm text-muted-foreground mt-2">
              Showing {totalVenues} venues with laser tag matching "{searchTerm}
              " in {displayCity}
            </p>
          )}
        </div>
      ...
      {!selectedState ? (
        ...
        <StateSelector
          ...
          onStateClick={(state) => {
            ...
            setLocation(`/laser-tag/${state.replace(/\s+/g, "-")}`);
          }}
          icon="🎯"
          testIdPrefix="lasertag-state"
          description="Click to browse laser tag venues"
          emptyStateTitle="No bowling & laser tag venues found"
          emptyStateMessage="We're still building our database of bowling alleys with laser tag."
          emptyStateIcon="🎯"
        />
      ) : !selectedCity ? (
        ...
        venuesLoading ? (
            ...
            {citiesInState.map((city) => (
              <Card
                ...
                onClick={() => {
                  ...
                  setLocation(`/laser-tag/${selectedState}/${city.slug}`);
                }}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Click to browse laser tag venues
                      </p>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">
              No cities with laser tag found
            </h3>
            <p className="text-muted-foreground">
              No bowling & laser tag venues found in {displayState}.
            </p>
          </div>
        )
      ...
      venuesLoading ? (
        ...
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🎯</div>
          <h3 className="text-lg font-semibold mb-2">
            No laser tag venues found
          </h3>
          <p className="text-muted-foreground">
            {searchTerm
              ? `No laser tag venues matching "${searchTerm}" in ${displayCity}`
              : `No bowling & laser tag venues found in ${displayCity}`}
          </p>
        </div>
      )}
